...
Bit's [Package Manager Aspect](/packages/overview) encapsulates [Yarn](/aspects/yarn), [pnpm](/aspects/pnmp), [NPM](/aspects/npm) or any other [supported CommonJS package manager](package-manager/) using the `bit install` command.
