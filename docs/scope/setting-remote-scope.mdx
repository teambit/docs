...
<!--TODO To explore popular use-cases of remote scopes, [see here](/what-is-bit#popular-use-cases).
 -->

Setting up a remote scope is done in two steps:

1. Create a scope on [Bit.dev](https://bit.dev) or [self-host](/getting-started/release-components/create-remote-scope/self-host-scope) on your own server.
...
To create a new remote scope on [Bit.dev](https://bit.dev), [follow these steps](/getting-started/release-components/create-remote-scope/host-on-bit-cloud).
...
The default scope is defined in the `workspace.jsonc` inside the `teambit.workspace/workspace` field. The default scope will be overridden by more [specific scope configurations](/getting-started/release-components/create-remote-scope/host-on-bit-cloud#setting-multiple-default-scopes).
...
A single workspace can export components to multiple scopes. To achieve that, use [@teambit.workspace/variants](/workspace/variants#merging-configurations) (in the `workspace.jsonc`)
...
To learn more about using `@teambit.workspace/variants` to select components, [see here](/workspace/variants#merging-configurations).
