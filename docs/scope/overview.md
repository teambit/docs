...
A remote scope is a remote collection of independent components that were 'snapped'/'tagged' and 'exported' from one or more [Bit workspaces](/workspace/overview). Storing components on a remote scope makes them available to be consumed and further maintained, by other Bit workspaces (in various repositories).
...
Remote scopes are hosted on [Bit.dev](https://bit.dev) or self-hosted Bit servers. Each Bit server can host multiple scopes.
