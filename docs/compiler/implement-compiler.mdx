...
---
All the compilation process and interfaces are defined under the [`teambit.compilations/compiler` aspect](https://bit.dev/teambit/compilation/aspect-docs/compiler)

The easiest way to start implementing a compiler is to create a new bit aspect using the `Aspect` template.

<!--TODO add links to real compilers like babel/ts -->
