...
A [Workspace](/workspace/overview) can be configured in the `workspace.jsonc` file, where each Aspect used in the Workspace can be configured according to its accepted configuration.

Each entry in the `workspace.jsonc` file is a definition of an Aspect, where the key is a Component ID of the used Aspect, and the value its his configuration.
...
At the above example, we are defining the [Workspace](/workspace/overview) Aspect to be configured with a specific `name` and an `icon`. In the same sense, Core Aspects can be configured and External Aspects can be used.
