...
BFF (Backend-For-Frontend) can be seen as an extension to the API Gateway pattern. BFF provides a layer for the frontend that handles data-fetching and further decouples the frontend concerns from the backend, allowing frontend developers focus on rendering data, consistency and frontend performance.  
