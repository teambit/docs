...
Bit is a monorepo tool for components. Teams use Git to collaborate on a shared Bit workspace with any number of components. Each component in a repository will have its own independent version and build pipeline. By maitnaining closely related components in the same repository a team can quickly iterate on changes and rely on Bit to propagate changes locally to 
