...
Collaborating between Bit components requires setting up a remote server accessible for all collaborators. You may use bit.dev as a remote server or set up your own.
...
A Bit server is a host server that hosts one or more bare scopes. That is, scopes that do not have a workspace (a working directory). Bit communication to the server is done using SSH protocol. Collaborators set up the server's scopes as remotes to their local scopes. Then, they export to and import from those scopes.
...
- Create a user for bit on your server and assign an SSH key to it
...
When logging into a server, Bit tries to log in using SSH with the following combinations:
...
1. Anonymous login. This happens only for read operations: username: `anonymous` and password is blank
...
The function takes the destination scope name (the destination from to get the dependency) and source scope name (the scope that has the dependent) as parameters. It should return a valid url for the destination, e.g: `file:///tmp/my-scope-name`. The example bellow is code taken from bit.dev:
...
To install packages to the registry, you will need to run code similar to the one used in [bit e2e tests](https://github.com/teambit/bit/blob/master/e2e/npm-ci-registry.ts).
