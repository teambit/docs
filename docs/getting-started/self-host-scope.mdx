---
id: self-host-scope
title: Set a Self-hosted Scope
---

Bit components can be hosted on the [Bit cloud](https://www.bit.dev) or can be self-hosted.
...
:::note
Here we are publishing to [npmjs.org](https://npmjs.org), but it's also possible to publish to Artifactory, Nexus or Verddacio.
:::
...
In order to publish components they must be tagged by using the `bit tag` command. Bit will run all tests and build a new version of each component as well as generate a `package.json` file for each one. It is during this process that your components get published to npm.
...
After tagging is complete components can be exported to your remote scope on your self hosted server using the `bit export` command.
...
Your components are now visible on npm as well as on your remote scope and can now be installed to any project or application using the package manager of your choice.

```bash
npm install @scope/component-name
yarn add @scope/component-name
```
