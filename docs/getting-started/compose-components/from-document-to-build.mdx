...
```js title=my-component.spec.tsx
import React from 'react';
...
```bash title="Test all Components"
bit test
```
...
```bash title="Watch mode for the CLI"
bit test --watch
```
...
Bit uses ESLint to lint your components. To use ESLint run the following command:
...
Learn how to configure the linter in the [Lint guide](/).
...
Learn how to configure the linter in the [Formatting guide](/).
