...
By default, Component compilation is done with the TypeScript compiler. The target format is `ES2015` which
supports execution from both NodeJS and browser runtimes for server-side rendering. The TypeScript compiler will compile both TypeScript and JavaScript components.
...
:::warning
@ran what is the difference between build and compile?
:::
