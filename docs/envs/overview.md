...
An Env is a development environment encapsulated in an independent component. Just like other components, development environments can be instantly added to your workspace to start developing, testing, and building components with zero configurations.
Different environments can be easily applied to multiple components in a workspace, and can be extended or customized to fit your needs and technologies. For example, try the React environment to start developing React components in your workspace.
Like all independent components, envs are reusable, so you can share them across projects and teams to greatly speed up and standardize development.

#### Example

```json
// Using the 'Node' and 'React' envs for different components in a Bit workspace.
{
