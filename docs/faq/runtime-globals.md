...
---
## Background
The JS compilation process often invloves adding c to the output compiled code. These polyfills are required and added only at the compilation phase so as to reduce duplication in the compiled code (which could be the result of importing them into every dependent module).
