...
```shell
# Imports all components in the remote scope
bit import "<owner>.<scope>/*"

# Imports all component under the namespace in a remote scope
...
- Update the workspace top level `package.json` to include the new component as a dependency. This allows importing the component in the code as `'@bit.<owner>.<scope>.<namespace>.<component-name>'
