...
ReactNative implements a set of APIs you can use to merge you prefered configuration with its defaults. These APIs are called **transformers** and they all start with the `override` pre-fix. Find all [Available transformers here](#transformers-api-docs).  
...
export class CustomReactNativeExtension {
  constructor(private reactNative: ReacNativetMain) {}
