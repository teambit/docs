...
Shared libraries (such as Bootstrap or Material UI) are examples of single repositories holding multiple components. Similarly, organizations manage their design systems in a single code repository.  

The result is a disconnect between producers and consumers. A producer stores all of the components in a single repository and a single package, while consumers of the components only need a subset of the package.  
...
Bit stores component information in scopes (in [bit.dev](https://bit.dev) they are referred to as **collections**). Just like a version control system stores related files in a project, a scope stores related components.  
...
A Bit workspace is a collection that resides inside a project. A workspace contains all of the component information and provides the functionality to author, export, import, and install components. Usually, a project has a single workspace.  
...
The `package.json` file has a `bit` section with all of the workspace configuration. Alternatively, a `bit.json` file at the project's root directory can store the information in a separate file. This configuration is editable by the user to fit the exact configuration of the project.  
...
- **collection**   the workspace's [collection](#collection) contains the content of components. By default, the component store is an extension to the git repository under `.git/bit` directory, but can be stored elsewhere, such as under a `.bit` folder.
...
A remote collection is a **bare collection** that only stores the component information. Authoring and tracking components are not possible inside a bare collection.  

A project's workspace exports components to a remote collection hosted on a Bit server.  

There are no limitations on exporting components - a local workspace may import from and export to multiple collections stored on different servers.  

Managing remote collections as well as local workspaces is done via the [`bit-cli`](#bit-cli-bit-bin). The server also needs to have the bit-cli and npm configurations.

For hosting remote collections you can use the [bit.dev cloud server](/docs/bit-dev) or setup your own [Bit Server](/docs/bit-server).

## Bit CLI (bit-bin)

Working with a bit workspace on a local dev machine or setting up a bit server requires the bit-cli tool. bit-cli manages the components and collections and their content.  

The bit-cli tool is an [open-source tool](https://github.com/teambit/bit). The tool is installed both in a project's workspace and on a Bit server for managing remote collections. 

All component and collection management is done using the bit-cli tool installed on a specific machine.  
