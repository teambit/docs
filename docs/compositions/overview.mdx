...
A component's 'composition' is the rendering of that component in a certain state and context. For non-UI components, compositions are used to visualize the component's I/O and its effects (for example, React hooks will use visual components to demonstrate their behavior in a visual way).
